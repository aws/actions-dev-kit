...
Using ADK SDK, customers can interact with CodeCatalyst concepts and resources. Let's start writing S3 Publish Action using ADK SDK.
...
>> adk publish --action-name <ACTION-NAME> --version 1.0.0
validating...
...
* 3. validate - Validates the action definition and README file. In the future, expand this command to validate the action is ready to be published.
...
* 5. generate-readme - Generate good quality README file for the action developer. This forms basis of what we want from README files to look like for CodeCatalyst actions.
...
* 8. publish - Publish the action from local workspace to CodeCatalyst. Does validate that the action version can be published.
* 9. deprecate/unpublish - Once we have the deprecate capabilitiy, tries to unpublish an action version or marks an action version ready to publish.
...
- Add at least 2 reviewers from the reviewer section. Reviewers can be anyone within the organization, but at least one developer from AEF team.
