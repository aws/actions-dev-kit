...
import { isString } from '@aws/codecatalyst-adk-utils';
...
function getAttributions(packageName: string, depth = 0, seen: Set<string> = new Set()): string {
  ...
  if (!PERMISSIBLE_DISTRIBUTION_LICENSES.includes(licenseId)) {
    throw new Error(`License ${licenseId} for package ${packageName} is not a permissible third-party license for distribution.`);
  }
...
function loadPackageJsonInfo(packageRoot: string): { dependencies: string[]; version: string; licenseId: string } {
  const packageJsonPath = path.join(packageRoot, 'package.json');
  const packageJson = require(packageJsonPath);
  const license = packageJson.license && isString(packageJson.license) ? packageJson.license : packageJson.license.type;
  return {
    dependencies: Object.keys(packageJson.dependencies || {}).filter(dep => !DEPENDENCIES_WITHOUT_LICENSE_FILE.includes(dep)),
    version: packageJson.version,
    licenseId: license,
  };
}

function getSingleAttribution(packageName: string, version: string, licenseId: string, licensePath: string, noticePath?: string) {
