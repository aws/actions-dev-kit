...
import { Model, parseAndSanitizeYamlFile } from '../src';
...
describe('Action.yaml Sanitizer Test', () => {
  it('should escape all strings in action.yml', async () => {
    ...
    expect(sanitizedResponseFiltersInputValue!.Description === `%0Aconsole.log('INJECTED_CODE')_${ESCAPED_INPUT_WITH_YAML_JS_ESCAPING}`).toBeTruthy();

    expect(sanitizedResponseFiltersInputValue!.DisplayName === `Response Filters with injection_${ESCAPED_INPUT}`).toBeTruthy();

    expect(sanitizedResponseFiltersInputValue!.Type === `string_${ESCAPED_INPUT}`).toBeTruthy();

    expect(sanitizedResponseFiltersInputValue!.Default === `${ESCAPED_INPUT}`).toBeTruthy();
  });
