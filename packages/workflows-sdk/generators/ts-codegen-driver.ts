...
void (async () => {
  ...
  for (const inputSchemaFile of walkFiles(modelsEntrypoint, ["**/*.json"])) {
    const outputTypescriptFile = path.join(
      typescriptSDK,
      path.dirname(inputSchemaFile),
      path.basename(inputSchemaFile).replace(".json", ".ts")
    );
    const schema = JSON.parse(
      fs.readFileSync(path.join(modelsEntrypoint, inputSchemaFile)).toString()
    );
