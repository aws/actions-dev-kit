...
describe('Init Command Tests', () => {
  describe('Init validation tests', () => {
    it('should fail validation if organization input params are not valid', async () => {
      ...
      validateWithInput(1, productInfo);
    });
    ...
    it('should fail validation if invalid language is passed', async () => {
      ...
      mockFS.existsSync
         .mockReturnValueOnce(false) // git
         .mockReturnValueOnce(true);
      validateWithInput(1, productInfo, 'test', 'java');
    });
  ...
  describe('Init success tests', () => {
    it("should succeed when all information is passed", () => {
      mockFS.existsSync
         .mockReturnValueOnce(true) // git
         .mockReturnValueOnce(false);
      validateWithMockData();
    });

    it("should ignore missing git configuration when disconnected flag is set", () => {
      mockFS.existsSync
         .mockReturnValueOnce(false) // git
         .mockReturnValueOnce(false);
      validateWithMockData(true);
    });
  });
